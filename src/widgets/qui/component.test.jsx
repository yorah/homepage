...
describe("widgets/qui/component", () => {
  ...
  it("falls back to stats when counts are absent", () => {
    // No counts block: seed/leech totals and total/errored come from stats instead.
    useWidgetAPI.mockReturnValue({
      data: {
        stats: { downloading: 2, seeding: 5, totalDownloadSpeed: 10, totalUploadSpeed: 20, total: 100, error: 4 },
      },
      error: undefined,
    });

    const service = { widget: { type: "qui", fields: ["leech", "seed", "total", "errored"] } };
    const { container } = renderWithProviders(<Component service={service} />, { settings: { hideErrors: false } });

    expectBlockValue(container, "qui.leech", "2 / 2"); // activeDl / stats.downloading fallback
    expectBlockValue(container, "qui.seed", "5 / 5"); // activeUp / stats.seeding fallback
    expectBlockValue(container, "qui.total", "100"); // stats.total fallback
    expectBlockValue(container, "qui.errored", "4"); // stats.error fallback
  });

  it("treats an empty instance string as aggregated", () => {
    useWidgetAPI.mockReturnValue({ data: aggregatedData, error: undefined });

    renderWithProviders(<Component service={{ widget: { type: "qui", instance: "" } }} />, {
      settings: { hideErrors: false },
    });
  ...
  it("shows placeholders when data has no stats yet", () => {
    useWidgetAPI.mockReturnValue({ data: { counts: { status: {} } }, error: undefined });
